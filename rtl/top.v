...
asynch_fifo_mem #(.DEPTH(DEPTH),.ADDR_WIDTH(ADDR_WIDTH),.DATA_WIDTH(DATA_WIDTH)) mem (.wr_clk(clk),.wr_rst_n(wr_rst_n),.full(full),.bin_wr_addr(wr_addr),.wr_data(wr_data),.bin_rd_addr(rd_addr),.rd_data(rd_data));
